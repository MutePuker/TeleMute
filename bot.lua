...
-- flood time/pm
  ...
  tdcli_function ({
	...
	local hash = 'floodlimit:max:'..msg.chat_id_
    ...
    local hash = 'floodlimit:time:'..msg.chat_id_
    ...
    if not is_momod(msg.sender_user_id_, msg.chat_id_) then
        local hash2 = 'anti-flood:'..msg.chat_id_
        if not redis:get(hash2) then
                if not is_owner(msg.sender_user_id_, msg.chat_id_) then
                    local hash = 'floodlimit:'..msg.sender_user_id_..':'..msg.chat_id_..':msgnum'
                    ...
                    if msgs > (floodMax - 1) then
						...
						local banhash =  'bot:banned:'..msg.chat_id_
                           redis:sadd(banhash, user_id)
                           tdcli.sendText(msg.chat_id_, msg.id_, 1, 'کاربر '..msg.sender_user_id_..' به دلیل فلود اخراج شد !', 1, 'md')
					  end
                    end
